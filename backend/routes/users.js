...
import { userCreateSchema, userProfileSchema, userUpdateSchema } from '../validation/schemas.js'
...
router.put('/profile', protect, validateBody(userProfileSchema), async (req, res) => {
  try {
...
router.post('/', protect, authorize('admin'), validateBody(userCreateSchema), async (req, res) => {
  try {
    const body = { ...req.body }
    if (!body.doctorRef || body.doctorRef === '') body.doctorRef = null
    if (body.doctorRef && !mongoose.Types.ObjectId.isValid(body.doctorRef)) {
...
router.put('/:id', protect, authorize('admin'), validateObjectId('id'), validateBody(userUpdateSchema), async (req, res) => {
  try {
    const body = { ...req.body }
    if (!body.doctorRef || body.doctorRef === '') body.doctorRef = null
    if (body.doctorRef && !mongoose.Types.ObjectId.isValid(body.doctorRef)) {
