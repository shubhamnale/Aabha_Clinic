    ...
    <div className="login-page" style={{ minHeight:'100vh', background:'linear-gradient(135deg,#0A2342 0%,#0D47A1 60%,#1565C0 100%)', display:'flex', alignItems:'center', justifyContent:'center', position:'relative', overflow:'hidden' }}>
      ...
      <div className="login-card" style={{ background:'rgba(255,255,255,.05)', backdropFilter:'blur(20px)', border:'1px solid rgba(255,255,255,.1)', borderRadius:24, padding:'48px 44px', width:'100%', maxWidth:420, margin:'0 16px', animation:'fadeIn .5s ease' }}>
        {onBack && (
          <button
            onClick={onBack}
            style={{ background:'transparent', border:'none', color:'rgba(255,255,255,.7)', fontSize:12, cursor:'pointer', marginBottom:16 }}
          >
            Back to website
          </button>
        )}
        <div style={{ textAlign:'center', marginBottom:36 }}>
          <img
  src={logo}
  alt="Aabha Trusted Gynaecology Care logo"
  style={{
    width: 74,
    height: 74,
    objectFit: 'contain',
...
/>
          <h1 className="login-title" style={{ fontFamily:"'Playfair Display',serif", fontSize:28, color:'#fff' }}>Aabha Trusted Gynaecology Care</h1>
          <p className="login-subtitle" style={{ color:'rgba(255,255,255,.5)', fontSize:13, marginTop:6 }}>Clinic Management System</p>
